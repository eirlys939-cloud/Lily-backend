...
【BP3 v2 滚动压缩说明（分桶版）】
- 触发条件：总消息数 >= BP3_COMPRESS_THRESHOLD（默认 60）
- 压缩策略：把对话切成稳定的桶（每桶 BP3_BUCKET_SIZE 条，默认 30）
            桶边界由消息序号决定，一旦划定永不变 → 桶内容稳定 → 指纹稳定 → 缓存稳定
- 摘要模型：claude-haiku-4-5-20251001
- 摘要呈现：每个桶作为独立的一对 user/assistant 伪消息塞在 messages 前面，
            活动窗口（未封桶的尾部）原样保留
- 摘要去向：① 替换 messages 前缀
            ② fire-and-forget 调 Celyn's Memory hold 永久存档（仅首次生成）

【A 方案：分桶摘要复用缓存】
- 核心洞察：桶的内容只跟"消息序号范围"有关，与"对话总长度"无关
            → 桶 1（第1~30条）永远是桶 1，指纹永远一致
            → 桶 2（第31~60条）永远是桶 2，指纹永远一致
- 缓存：进程内 TTL+LRU，key=桶内容的 SHA256
- TTL：24 小时；容量：100 条（LRU 淘汰）
- Haiku 调用次数 ≈ 桶数（每个桶一辈子只摘要一次）
...
BP3_COMPRESS_THRESHOLD = 60       # 总消息数 >= 此值才开始分桶压缩
BP3_BUCKET_SIZE = 30              # 每个桶装 30 条消息，桶边界一旦划定永不变
BP3_SUMMARY_MODEL = "claude-haiku-4-5-20251001"
BP3_SUMMARY_MAX_TOKENS = 2000     # 单个桶摘要的 max_tokens

# A 方案：分桶摘要复用缓存（进程内 TTL + LRU）
# key = 桶内容的 SHA256；value = (写入时间戳, 摘要文本)
# 桶内容稳定 → 指纹稳定 → 缓存稳定命中
BP3_CACHE_TTL = 86400             # 摘要缓存有效期（秒），24 小时
BP3_CACHE_MAX = 100               # 缓存条目上限，超过按 LRU 淘汰最老
_BP3_SUMMARY_CACHE: dict[str, tuple[float, str]] = {}
...
async def _compress_if_needed(
    ...
    """
    BP3 v2 分桶压缩入口。返回 (新的 convo, 压缩信息 or None)。

    核心算法：
    1. 总消息数 N < 60，不压缩，原样返回
    2. 计算"已封桶部分"长度：sealed = ((N - BUCKET_SIZE) // BUCKET_SIZE) * BUCKET_SIZE
       例：N=95, sealed=60（2 个桶）；N=120, sealed=90（3 个桶）
    3. 切桶：[桶1: 0~30] [桶2: 30~60] ... [活动: sealed~N]
    4. 每个桶独立查缓存 / 调 Haiku → 拿到摘要
    5. 拼装：每个桶 → 一对 user/assistant 伪消息；活动窗口原样保留

    压缩信息字典格式：
        {
            "triggered": True,
            "original_count": 95,
            "compressed_count": 39,  # 2桶*2消息 + 35活动消息
            "buckets_total": 2,
            "buckets_cache_hit": 2,
            "buckets_new": 0,
            "summary_chars_total": 1234,
        }
    """
    n = len(convo)
    if n < BP3_COMPRESS_THRESHOLD:
        return convo, None

    # 计算已封桶部分长度（必须是 BUCKET_SIZE 的整倍数，且至少留一个活动窗口）
    sealed = ((n - BP3_BUCKET_SIZE) // BP3_BUCKET_SIZE) * BP3_BUCKET_SIZE
    if sealed <= 0:
        return convo, None

    # 切桶
    buckets: list[list[dict[str, Any]]] = []
    for start in range(0, sealed, BP3_BUCKET_SIZE):
        buckets.append(convo[start:start + BP3_BUCKET_SIZE])
    active_window = convo[sealed:]

    # 安全检查：活动窗口第一条必须是 user（Anthropic 要求 assistant 后接 user）
    # 由于 BUCKET_SIZE=30 是偶数且对话严格 user/assistant 交替，这里应该天然满足
    if not active_window or active_window[0].get("role") != "user":
        log.warning(
            f"BP3 v2: 活动窗口起点不是 user 角色（sealed={sealed}），"
            f"跳过本次压缩以避免破坏消息序列"
        )
        return convo, None

    log.info(
        f"🗜️  BP3 v2 触发：总 {n} 条 → 切 {len(buckets)} 桶 "
        f"+ 活动窗口 {len(active_window)} 条"
    )
    ...
    for i, bucket in enumerate(buckets, start=1):
        fp = _fingerprint_messages(bucket)
        cached = _cache_get(fp)
        if cached is not None:
            bucket_summaries.append(cached)
            cache_hits += 1
            log.info(
                f"♻️  桶 {i}/{len(buckets)} 命中缓存（key={fp[:12]}…，"
                f"累计 hits={_BP3_CACHE_HITS}/misses={_BP3_CACHE_MISSES}）"
            )
        else:
            summary = await _summarize_old_messages(bucket)
            if not summary:
                log.warning(f"BP3 v2: 桶 {i}/{len(buckets)} 摘要失败，跳过整次压缩")
                failed = True
                break
            _cache_set(fp, summary)
            bucket_summaries.append(summary)
            cache_news += 1
            log.info(
                f"🆕 桶 {i}/{len(buckets)} 新摘要 {len(summary)} 字符（key={fp[:12]}…，"
                f"cache_size={len(_BP3_SUMMARY_CACHE)}）"
            )
            # fire-and-forget：新摘要存进 Celyn's Memory
            asyncio.create_task(_save_summary_to_memory(summary, len(bucket), memory_client))

    if failed:
        return convo, None

    # 拼装新 convo：每个桶 → 一对 user/assistant 伪消息
    new_convo: list[dict[str, Any]] = []
    total_chars = 0
    for i, summary in enumerate(bucket_summaries, start=1):
        new_convo.append({
            "role": "user",
            "content": f"[历史段 {i}/{len(bucket_summaries)} 摘要]\n\n{summary}",
        })
        new_convo.append({
            "role": "assistant",
            "content": f"好的，我已经记住了历史段 {i} 的内容。",
        })
        total_chars += len(summary)

    new_convo.extend(active_window)

    log.info(
        f"✂️  BP3 v2 完成：{n} → {len(new_convo)} 条 "
        f"[♻️ 命中 {cache_hits} 桶 / 🆕 新 {cache_news} 桶 / 共 {total_chars} 字符]"
    )
    ...
    info = {
        ...
        "buckets_cache_hit": cache_hits,
        "buckets_new": cache_news,
        "summary_chars_total": total_chars,
    }
...
async def run_tool_loop(
    *,
    ...
    if compress_info:
        log.info(
            f"✂️  BP3 v2 已压缩：{compress_info['original_count']} → "
            f"{compress_info['compressed_count']} 条 "
            f"[{compress_info['buckets_total']} 桶: "
            f"♻️ {compress_info['buckets_cache_hit']} 命中 / "
            f"🆕 {compress_info['buckets_new']} 新摘要 / "
            f"共 {compress_info['summary_chars_total']} 字符]"
        )
