...
【BP3 滚动压缩说明】
- 触发条件：messages 数 >= BP3_COMPRESS_THRESHOLD（默认 40）
- 压缩策略：保留最近 BP3_KEEP_RECENT 条（默认 20），其余调 Haiku 总结
...
BP3_COMPRESS_THRESHOLD = 40       # messages 数 >= 这个值就触发压缩
BP3_KEEP_RECENT = 20              # 压缩时保留最近多少条
    ...
    *,
    ...
    if compress_info:
        log.info(
            ...
            f"{compress_info['compressed_count']} 条消息，摘要 {compress_info['summary_chars']} 字符"
        )
