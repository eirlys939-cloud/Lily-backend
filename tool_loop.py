...
async def _build_tool_specs(
    ...
    # ★ 在最后一个工具上打 cache_control —— 整个 tools 数组会被缓存
    #   ttl=1h：缓存延长到 1 小时（默认 5 分钟）
    if tools_for_api:
        tools_for_api[-1] = {
            **tools_for_api[-1],
...
async def run_tool_loop(
    *,
    ...
    def _apply_rolling_cache_breakpoint(msgs: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
        ★ BP4 rolling 断点：给倒数第二条 user 消息的最后一个 content block 打 cache_control。
        这会把缓存边界从 system 末尾扩展到包含所有历史对话。
        - 至少需要 2 条 user 消息才会打标（否则跟最新这条重叠，等于没打）
        - content 是字符串的 user 消息会被自动包装成 content block 数组以便打标
        - 不修改原列表，返回新列表
        """
        # 找所有 user 消息的索引
        ...
        target_idx = user_indices[-2]  # 倒数第二条 user
        result = list(msgs)
